              ...
              $bdd = new PDO("mysql:host=$host_name; dbname=$database;", $user_name, $password);
                  ...
                  if(isset($_POST['upload'])) {
                    $id=$_SESSION['id'];
                     ...
                     if(!empty($_POST['titre']) AND !empty($_POST['imatricule']) AND !empty($_POST['date_debut']) AND !empty($_POST['date_fin']) AND !empty($_POST['prix']) AND !empty($_POST['description'])) {
                                  ...
                                  if (isset($_FILES['file'])) {
                                       ...
                                       upload_image($file_ext,$file_destination,$file_error,$file_size,$file_tmp);
                                        ...
                                        $insertmbr = $bdd->prepare("INSERT INTO contrat_location_pro(id,titre,matricule,desciption,date_debut,date_fin,prix,image) VALUES(?,?,?,?,?,?,?,?)");
                                         $insertmbr->execute(array($id,$titre,$imatricule,$description,$date_debut,$date_fin,$prix,$file_destination));
                                         $erreur = "l'annonce a été ajouter, elle sera affiché dans une heure !";
                                }
                      } else {
...
<section style="maring-top :5%;">
         ...
         <article style=" color: white;background-color: rgba(0,0,0,0.5);margin-right:12%;margin-left:12%;margin-top :-8%; border-radius:10px 10px;" id="volet">
             <h1 style="text-align: center;font-family:arial; background-color: rgba(199,21,133, 0.8);padding-top:2%;padding-bottom:2%;border-radius:10px 10px 0px 0px;">Dépot d'annonce</h1>
              <p style="margin-left:90%;"><a href="./historique.php" style="color: white;">Historique</a></p>
         <form method="POST" action="" style="color:white;padding-top :5%;padding-bottom :10%; " enctype="multipart/form-data">
            <table>
                 <tr>
                  <td align="right">
               ...
               <tr>
                ...
                <tr>
                  <td align="right">
                      <label for='date_fin' class='labelDateFin'></label>
                  </td>
                  <td>
                      <input type="date" placeholder="date_debut" style="color:white; background-color: rgba(0, 0, 255, 0.1);width: 80%; height: 40px;border-radius:10px 10px; margin-bottom:2%;" name="date_debut" <?php echo'min="'.date('Y-m-d').'"';?> required/>
                  </td>
               </tr>
                <tr>
                  <td align="right">
                  ...
                  <td>
                      <input type="date" placeholder="date_fin" style="color:white; background-color: rgba(0, 0, 255, 0.1);width: 80%; height: 40px;border-radius:10px 10px; margin-bottom:2%;" name="date_fin" <?php echo'min="'.date('Y-m-d').'"';?> required/>
                  </td>
               </tr>
